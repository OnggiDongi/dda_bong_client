...
export default function ReviewListPage() {
  ...
  const {
    ...
    isError: isErrorRecruiting,
    error: errorRecruiting,
  } = useGetMyActivityPosts(true);
  const {
    data: pastPosts,
    isLoading: isLoadingPast,
    isError: isErrorPast,
    error: errorPast,
  } = useGetMyActivityPosts(false);
        ...
        {listData && listData.length > 0 ? (
          listData.map((item) => (
            <VolunList
              ...
              date={item.endAt ?? ''}
              category={
                CATEGORY_REVERSE_MAP[item.category as string] ||
                item.category ||
                ''
              }
